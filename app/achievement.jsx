...
export default function AchievementScreen() {
  ...
  async function shareToInstagram() {
    ...
    if (!opened) {
      // Fallback: native share sheet so user can copy and paste into Instagram
  ...
  async function shareToX() {
    ...
    if (!opened) await Linking.openURL(tweetUrl).catch(() => Share.share({ message: shareText }));
  }
  ...
  async function shareNative() {
    await Share.share({ message: shareText, title: 'My FBLA Achievement' });
  }
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <View style={s.body}>
        ...
        <Pressable style={s.moreBtn} onPress={shareNative}>
          <Ionicons name="share-outline" size={20} color={colors.primary} />
          <Text style={s.moreBtnText}>More sharing options…</Text>
        </Pressable>
      </View>
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  shareHeading:  { color: colors.primary, fontSize: fontSize.lg, fontWeight: '800', marginBottom: spacing.xl },
  ...
  moreBtn:       { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginTop: spacing.sm, paddingVertical: spacing.sm },
  moreBtnText:   { color: colors.primary, fontSize: fontSize.sm, textDecorationLine: 'underline' },
});
